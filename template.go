...
func ParseFiles(filenames ...string) (t *Template, err error) {
	if len(filenames) == 0 {
		return nil, fmt.Errorf("postgresql/template: no files named in call to ParseFiles")
	}
...
func ParseGlob(pattern string) (t *Template, err error) {
	var rootname string
	{
		...
		if len(filenames) == 0 {
			return nil, fmt.Errorf("postgresql/template: pattern matches no files: %#q", pattern)
		}
		rootname = filepath.Base(filenames[0])
	}
	t = New(rootname)
...
func ParseFS(fsys fs.FS, patterns ...string) (t *Template, err error) {
	if len(patterns) == 0 {
		return nil, fmt.Errorf("postgresql/template: no patterns in call to ParseFS")
	}
	var rootname string
	{
		...
		if len(filenames) == 0 {
			return nil, fmt.Errorf("postgresql/template: pattern matches no files: %#q", patterns[0])
		}
		rootname = filepath.Base(filenames[0])
	}
	t = New(rootname)
...
func (tpl *Template) ExecuteTemplate(q *Query, name string, data interface{}) error {
	if t := tpl.Lookup(name); t != nil {
		...
		return fmt.Errorf("postgresql/template: no template %q associated with template %q", name, tpl.Name())
	}
}
